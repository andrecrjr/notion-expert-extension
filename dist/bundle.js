...
var require_errors = __commonJS({
  "node_modules/@notionhq/client/build/src/errors.js"(exports) {
    ...
    var APIResponseError2 = class extends HTTPResponseError {
      constructor() {
        super(...arguments);
    ...
    exports.APIResponseError = APIResponseError2;
    function buildRequestError(response, bodyText) {
      const apiErrorResponseBody = parseAPIErrorResponseBody(bodyText);
      if (apiErrorResponseBody !== void 0) {
        return new APIResponseError2({
  ...
  // ==================== PAGES ====================
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        icon: {
          type: "string",
          description: 'Page icon (emoji like "\u{1F4DD}" or external URL)'
        },
        cover: {
          type: "string",
...
var SERVICE_NAME = "gemini-notion-extension";
...
var rateLimiter = new TokenBucket(3, 3);
function sleep(ms) {
...
async function withRetry(operation, maxRetries = 3, operationName = "API call") {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      if (error instanceof import_client.APIResponseError) {
    ...
    switch (name) {
      ...
      case "notion_search": {
        ...
        const formatted = results.results.map((r) => {
          ...
          if (r.object === "database") {
            ...
            const titleProp = props.title?.title || props.Name?.title || props.Title?.title || props.name?.title || props["Page Title"]?.title || [];
      ...
      case "create_page": {
        ...
        if (args?.parentDatabaseId) {
          ...
          if (args?.title && !properties.hasOwnProperty("title") && !properties.hasOwnProperty("Name") && !properties.hasOwnProperty("Title") && !properties.hasOwnProperty("name")) {
        ...
        const children = args?.content ? markdownToBlocks(args.content) : void 0;
        const page = await withRetry(() => notion.pages.create({
          parent,
          properties,
          children,
          icon: args?.icon ? args.icon.startsWith("http") ? { type: "external", external: { url: args.icon } } : { type: "emoji", emoji: args.icon } : void 0,
          cover: args?.cover ? { type: "external", external: { url: args.cover } } : void 0
        }), 3, "create_page");
      ...
      case "update_page": {
        ...
        if (args?.icon) {
          updates.icon = args.icon.startsWith("http") ? { type: "external", external: { url: args.icon } } : { type: "emoji", emoji: args.icon };
        }
        if (args?.cover) {
          updates.cover = { type: "external", external: { url: args.cover } };
        }
        if (args?.archived !== void 0) {
      ...
      case "get_database_schema": {
        ...
        const schema = Object.entries(db.properties).map(([name2, prop]) => ({
          name: name2,
          type: prop.type,
          options: prop.select?.options || prop.multi_select?.options || void 0
        }));
        return respond({
          id: db.id,
          title: richTextToPlain(db.title),
      ...
      case "list_projects": {
        ...
        const filter = args?.status ? { property: "Status", select: { equals: args.status } } : void 0;
        const sorts = args?.sortBy === "date" ? [{ property: "Start Date", direction: "descending" }] : args?.sortBy === "status" ? [{ property: "Status", direction: "ascending" }] : [{ property: "Project Name", direction: "ascending" }];
      ...
      case "export_conversation": {
        ...
        const title = args?.title || messages.find((m) => m.role === "user")?.content?.slice(0, 50) + "..." || "Gemini Conversation";
        const blocks = [];
        for (const msg of messages) {
          blocks.push({
            ...
            type: "callout",
            callout: {
              icon: { type: "emoji", emoji: msg.role === "user" ? "\u{1F464}" : "\u{1F916}" },
              rich_text: [{ type: "text", text: { content: msg.content.slice(0, 2e3) } }]
            }
          });
        }
        const properties = {
          "Title": { title: [{ text: { content: title } }] },
          "Export Date": { date: { start: (/* @__PURE__ */ new Date()).toISOString() } },
      ...
      case "get_recent_changes": {
        ...
        const formatted = results.results.map((r) => ({
          id: r.id,
          type: r.object,
          title: r.object === "database" ? richTextToPlain(r.title) : richTextToPlain(r.properties?.title?.title || r.properties?.Name?.title || []),
          lastEdited: r.last_edited_time,
          url: r.url
        }));
        return respond({ count: formatted.length, results: formatted });
      ...
      case "create_from_template": {
        ...
        const parent = parentDatabaseId ? { database_id: resolveDatabaseId(parentDatabaseId) } : { page_id: parsePageId(parentPageId) };
        const properties = parentDatabaseId ? { Name: { title: [{ text: { content: title } }] } } : { title: { title: [{ text: { content: title } }] } };
        const page = await withRetry(() => notion.pages.create({
          parent,
      ...
      case "check_api_health": {
        ...
        if (includeDetails) {
          Object.assign(health, {
            ...
            uptimeSeconds: Math.floor((Date.now() - (global.serverStartTime || Date.now())) / 1e3)
          });
        }
        return respond(health);
      }
      ...
      case "bulk_create_pages_from_files": {
        let scanFolderForFiles2 = function(folderPath2, extensions) {
          const files2 = [];
          try {
            const items = readdirSync(folderPath2);
            for (const item of items) {
              const itemPath = join(folderPath2, item);
              const stat = statSync(itemPath);
              if (stat.isFile()) {
                const ext = extname(item).toLowerCase();
                if (extensions.includes(ext)) {
                  files2.push(itemPath);
                }
              }
            }
          } catch (error2) {
            console.error(`Error scanning folder ${folderPath2}:`, error2);
          }
          return files2;
        };
        var scanFolderForFiles = scanFolderForFiles2;
        const folderPath = args?.folderPath;
        const databaseId = resolveDatabaseId(args?.databaseId);
        const fileExtensions = args?.fileExtensions || [".pdf"];
        const files = scanFolderForFiles2(folderPath, fileExtensions);
        let successCount = 0;
        let errorCount = 0;
