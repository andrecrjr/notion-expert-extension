...
let notion: Client;
let dataSourceResolver: DataSourceResolver;
      ...
      for (const [key, prop] of Object.entries(page.properties) as [string, any][]) {
        switch (prop.type) {
          ...
          case 'relation':
            ...
            break;
          case 'people':
            formatted[key] = prop.people?.map((p: any) => p.id);
            break;
          case 'files':
            formatted[key] = prop.files?.map((f: any) => f.name || f.file?.url || f.external?.url);
            break;
          case 'formula':
...
const server = new Server(
  { name: 'notion-sync-enhanced', version: '3.0.1' },
  { capabilities: { tools: {} } }
);
    ...
    switch (name) {
      ...
      case 'notion_search': {
        const filter = args?.filter === 'all' ? undefined :
          args?.filter ? { property: 'object' as const, value: args.filter as 'page' | 'data_source' } : undefined;
      ...
      case 'create_page': {
        ...
        if (args?.parentDatabaseId) {
          ...
          const resolvedPageId = parsePageId(args.parentPageId as string);
      ...
      case 'create_database': {
        ...
        const db = await withRetry(
          () => notion.databases.create({
            ...
            initial_data_source: { properties: args?.properties as any || defaultProperties },
          }),
          3,
          'create_database'
        );
            ...
            'Document Type': {
              select: {
                options: [
                  ...
                  { name: 'Contract', color: 'red' as const },
                  { name: 'Scan', color: 'purple' as const }
                ]
              }
            },
            'Status': {
              select: {
                options: [
                  ...
                  { name: 'Processed', color: 'green' as const },
            ...
            'Status': {
              select: {
                options: [
                  ...
                  { name: 'On Hold', color: 'red' as const }
                ]
              }
            },
            ...
            'Meeting Type': {
              select: {
                options: [
                  ...
                  { name: 'Client Meeting', color: 'green' as const },
                  { name: 'One-on-One', color: 'purple' as const },
                  { name: 'All Hands', color: 'red' as const }
                ]
              }
            },
            ...
            'Status': {
              select: {
                options: [
                  ...
                  { name: 'Blocked', color: 'gray' as const }
                ]
              }
            },
            'Priority': {
              select: {
                options: [
                  ...
                  { name: 'Medium', color: 'yellow' as const },
                  { name: 'High', color: 'red' as const },
                  { name: 'Critical', color: 'purple' as const }
                ]
              }
            },
            'Assignee': { rich_text: {} },
